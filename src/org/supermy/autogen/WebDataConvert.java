...
public class WebDataConvert {
	...
	public String valid(Field field) {
		...
		if(field.isAnnotationPresent(Digits.class)){//数字是否符合整数部分和小数部分的精度 ,定义列精度和范围
		...
		if(field.isAnnotationPresent(Email.class) || field.getName().toLowerCase().endsWith("email")){
			valid.append(",email:true");
		}
		if( field.getName().toLowerCase().endsWith("url")|| field.getName().toLowerCase().contains("http")){
		...
		if(field.isAnnotationPresent(Length.class)){
			...
			valid.append(",maxlength:").append(annotation.max());
		...
		if(field.isAnnotationPresent(Range.class)){
			...
			valid.append(",range:[").append(annotation.min()).append(",").append(annotation.max()).append("]");
		}
