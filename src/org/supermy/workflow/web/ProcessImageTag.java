 ...
 */
package org.supermy.workflow.web;
...
import org.supermy.workflow.service.ImageService;
...
public class ProcessImageTag extends RequestContextAwareTag {
	...
	private void retrieveByteArrays() {
		...
		try {
			...
			gpdBytes = imageService.getGpdBytes(taskInstanceId);

			imageBytes = imageService.getImageBytes(taskInstanceId);
	...
	private void writeTable() throws IOException, DocumentException {
		...
		String imageLink = ((HttpServletRequest) pageContext.getRequest())
				.getContextPath()
				+ "/workflow/workflow!workflowImage.action?id="
				+ imageService.getProcessDefinitionID(taskInstanceId);
		...
		if (tokenInstanceId > 0 && tokenInstanceId != 0) {
			...
			allTokens = imageService.walkTokens(allTokens, taskInstanceId);
...

			{
				...
				if (i > 0) {
					...
					while (offset > 0
							&& (imageService.getParentToken((Token) allTokens
									.get(offset - 1))).equals(imageService
				...
				boxConstraint = imageService.extractBoxConstraint(
						rootDiagramElement, token);
				...
				"width: " + boxConstraint[2] + "px; height: "
						+ boxConstraint[3] + "px;'>");
...

		{
			...
			boxConstraint = imageService.extractBoxConstraint(
					rootDiagramElement, taskInstanceId);
	 ...
	 * 
	 * currentToken =
	 * jbpmContext.getGraphSession().loadToken(this.tokenInstanceId);
	 * 
	 * }
	 * 
	 * processDefinition =
	...
	protected int doStartTagInternal() throws Exception {
		...
		imageService = (ImageService) this.getRequestContext()
				.getWebApplicationContext().getBean("imageService");
